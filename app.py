...
def index():
    art_path = os.path.abspath(
        os.path.join(app.static_folder, "assets", "img", "art")
    )

    projects = []

    # Normalize artwork keys once
    artworks = {k.lower(): v for k, v in artworks_raw.items()}

    # Ensure deterministic order
    for folder in sorted(os.listdir(art_path), key=str.lower):
        ...
        project = {
            ...
            "sold": info.get("sold", False),
            "preview": f"assets/img/art/{folder_key}/{files[0]}",
            "files": files
        }
